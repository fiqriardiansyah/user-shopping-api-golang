...
func (p *Pages) LoginHandleForm(ctx *fiber.Ctx) error {
	...
	response, err := p.Auth.AuthUseCase.Login(ctx.UserContext(), request)
	if err != nil {
		params.Add("error", err.Error())
		params.Add("email", request.Email)
		u.RawQuery = params.Encode()
...
func (p *Pages) PageLogin(ctx *fiber.Ctx) error {
